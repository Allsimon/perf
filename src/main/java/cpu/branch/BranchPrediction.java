...
import static util.BenchUtils.throwsIfDifferent;
...
public class BranchPrediction {
  ...
  @Benchmark
  public int unsortedArray(BranchPredictionDataset dataset) {
    return sumOnArray(dataset.input);
  }

  @Benchmark
  public int sortedArray(BranchPredictionDataset dataset) {
    return sumOnArray(dataset.sortedInput);
  }

  private int sumOnArray(int[] array) {
    int sum = 0;
    for (int value : array) {
      if (value >= MIN_VALUE) {
        sum += value;
  ...
  public static class NoBranches {
    ...
    @Benchmark
    public int branchlessUnsortedArray(BranchPredictionDataset dataset) {
      return branchlessSumOnArray(dataset.input);
    }

    @Benchmark
    public int branchlessSortedArray(BranchPredictionDataset dataset) {
      return branchlessSumOnArray(dataset.sortedInput);
    }

    private int branchlessSumOnArray(int[] array) {
      int sum = 0;
      for (int value : array) {
  ...
  public static class StreamBranches {
    ...
    @Benchmark
    public int streamUnsortedArray(BranchPredictionDataset dataset) {
      return sumStream(dataset.input);
    }

    @Benchmark
    public int streamSortedArray(BranchPredictionDataset dataset) {
